...
function setAuth() {
	return { 'Authorization': 'Basic ' + Base64.encode( sessionStorage.getItem('login') + ':' + sessionStorage.getItem('password') ) };
}


function createMilestone(repo, repoOwner, name) {
	var url = 'https://api.github.com/repos/' + this.repoOwner + '/' + this.repo + '/milestones';
	$.ajax({
    url: url,
    type: 'POST',
